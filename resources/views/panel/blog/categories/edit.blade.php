...
@section('page-subtitle', 'Edite as informações da categoria')

@section('content')
<div class="max-w-2xl mx-auto">
    <form action="{{ route('panel.blog.categories.update', $category->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" 
                              ...
                              rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Breve descrição da categoria">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div class="flex items-center space-x-3 p-4 bg-gray-50 rounded-lg">
                ...
                <span id="preview-name" class="font-medium text-gray-900">{{ old('name', $category->name) }}</span>
                <span id="preview-status" class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ old('active', $category->active) ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                    {{ old('active', $category->active) ? 'Ativa' : 'Inativa' }}
                </span>
            </div>
        </div>
